...
ggplot(first_word_count[1:20,], aes(x=reorder(lowercaseLemma,n),y=n))+
...
Now Let's look two specific topics, Exercise and Pets
...
pet.dict <- read_csv('https://raw.githubusercontent.com/rit-public/HappyDB/master/happydb/data/topic_dict/pets-dict.csv', col_names = FALSE)
pet <- as.vector(pet.dict$X1)
pet.df <- bagOfWords %>% 
  filter(word %in% pet) %>% select(id) %>% unique() %>%
  left_join(one.line, by=c("id"="id"))
# age distribution
ggplot(data = pet.df, aes(x=age))+
  geom_density(aes(color="pet")) +
  # Change the fill colour to differentiate it
  geom_density(data=one.line,aes(color="overall")) +
  theme_bw()
```
Pets bring more happiness for people at the age of 50s. This may because people at this age have retired and do not live with their chidren anymore. Pets are good companions and friends of them.

```{r,message=FALSE,warning=FALSE,echo=FALSE}
# overall marital&parenthood status
single_rate_overall <- mean(one.line$marital=="single")
parenthoody_rate_overall <- mean(one.line$parenthood == "y")

# marital 
p1 <- ggplot(pet.df, aes(x=gender,fill=marital)) + geom_bar(position="fill") +
  theme_bw() + theme(legend.position = "top") +
  scale_y_continuous(labels = percent_format()) +
  geom_hline(yintercept = single_rate_overall,linetype="dashed",color="white")
# parenthood
p2 <- ggplot(pet.df, aes(x=gender,fill=parenthood)) + geom_bar(position="fill") +
  theme_bw() + theme(legend.position = "top") +
  scale_y_continuous(labels = percent_format()) +
  geom_hline(yintercept = parenthoody_rate_overall,linetype="dashed",color="white")
grid.arrange(p1, p2, nrow=1, ncol=2)
```

For those who shared happy moments with their pets, there are more men who are single or don't have kids. I think one of the mojor reason is that if a man get married or have children, his attention for pets gets lower and pay more attention on his fmaily.
